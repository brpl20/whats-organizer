...
export FLASK_PORT_START FLASK_PORT_END PORT UPSTREAM_SERVERS CLOUDFLARE_IPS_FIREWALL CLOUDFLARE_REAL_IP NGINX_LOCALHOST MAX_UPLOAD_MB

envsubst '$FLASK_PORT_START $FLASK_PORT_END $PORT $UPSTREAM_SERVERS $CLOUDFLARE_IPS_FIREWALL $CLOUDFLARE_REAL_IP $NGINX_LOCALHOST $MAX_UPLOAD_MB' < "${TEMPLATE_PATH}" > "${OUT_PATH}"
exit 0
