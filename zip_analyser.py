...
# Add this after you determine creation_systems and creation_tools
def determine_device_type(create_systems, creator_tools):
    """Determine if the zip was created on Android or iPhone based on system info"""
    
    # Check for Android (Windows/DOS patterns)
    if any(sys in [0, 10, 14] for sys in create_systems) or \
       any("Windows" in tool or "MS-DOS" in tool for tool in creator_tools):
        return "android"
        
    # Check for iPhone (macOS patterns)
    if 19 in create_systems or \
       any("macOS" in tool or "OS X" in tool for tool in creator_tools):
        return "iphone"
        
    # If can't determine specifically
    return "Unknown"
...
def analyze_zip_file(zip_path):
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            # Get comment if any
            comment = (
                zip_ref.comment.decode("utf-8", errors="replace")
                if zip_ref.comment
                else None
            )
            ...
            for file in file_list:
                ...
                whatsapp_pattern = re.search(
                    r"Conversa do WhatsApp com (.+)\.txt$", file
                )
                if whatsapp_pattern:
                    whatsapp_contact_name.append(whatsapp_pattern.group(1))
                # pdb.set_trace()
                ...
                for idx, part in enumerate(path_parts_list):
                    ...
                    username_pattern = re.search(
                        r"[/\\]Users[/\\]([^/\\]+)[/\\]", file, re.IGNORECASE
                    )
                    if username_pattern and username_pattern.group(1) not in [
                        "Public",
                        "Shared",
                        "Default",
                        "All Users",
            ...
            if suspicious_files:
                return (
                    f"SECURITY ALERT: Potential ZIP bomb detected - {len(suspicious_files)} files with suspicious compression ratios:\n"
        ...
        # Identificar Android ou Iphone Função:
        device_type = determine_device_type(create_systems, creator_tools)
        ...
        if username_patterns:
            ...
            username_analysis = (
                f"Potential usernames found in paths: {', '.join(unique_usernames)}\n"
            )
        ...
        result = {
            "file_info": {
                ...
                "full_path": os.path.abspath(zip_path),
                "contact": whatsapp_contact_name,
            ...
            "archive_info": {
                ...
                "total_uncompressed_formatted": format_size(total_uncompressed_size),
                "total_compressed_bytes": total_compressed_size,
                "total_compressed_formatted": format_size(total_compressed_size),
                "overall_compression_ratio": (
                    1 - total_compressed_size / total_uncompressed_size
                )
                * 100
                if total_uncompressed_size > 0
                else 0,
                "contains_directories": has_directories,
                "contains_encrypted_files": has_encrypted_files,
                "oldest_file_date": oldest_file_date.isoformat(),
                "newest_file_date": newest_file_date.isoformat(),
                "file_extensions": sorted(list(extensions)) if extensions else [],
                "compression_methods": sorted(list(compression_methods_used)),
                "creation_systems": [
                    get_system_name(sys) for sys in sorted(create_systems)
                ],
            },
            "hash_info": {"md5": md5_hash, "sha1": sha1_hash, "sha256": sha256_hash},
        }


        # Add origin analysis
        result["origin_analysis"] = {
            "creation_tools": list(creator_tools) if creator_tools else [],
            "embedded_creators": embedded_creators,
            "common_root_directories": common_prefixes if common_prefixes else [],
            "common_path_elements": common_paths if common_paths else [],
            "potential_usernames": list(set(username_patterns))
            if username_patterns
            else [],
        }
        ...
        # Add signature information if applicable
        result["signature_info"] = {
            "has_signatures": bool(signature_info),
            "details": signature_info
            if signature_info
            else "No digital signatures were detected",
        }
        ...
        for item in file_details:
            file_entry = {
                ...
                "compressed_bytes": item["compressed_size"],
                "compressed_formatted": format_size(item["compressed_size"]),
                ...
                "create_system": get_system_name(item["create_system"]),
                "extract_version": item["extract_version"],
                "flag_bits": bin(item["flag_bits"]),
                "volume": item["volume"],
                "extra_fields": binascii.hexlify(item["extra"]).decode("ascii")
                if item["extra"]
                else None,
                "internal_attributes": bin(item["internal_attr"]),
                "external_attributes": bin(item["external_attr"]),
            }
...
def check_for_signatures(zip_path):
    ...
    try:
        ...
        if sig_marker in data:
            ...
            signature_info = (
                "Found Central Directory Digital Signature at offset: "
                + str(pos)
                + "\n"
            )
            ...
            try:
                ...
                if sig_size > 0:
                    ...
                    if signature_data.startswith(b"\x30"):
                        signature_info += (
                            "  Signature appears to be in PKCS#7 or ASN.1 DER format\n"
                        )
...
def extract_cert_info(signature_data):
    ...
    try:
        ...
        cn_match = re.search(
            b"\x06\x03\x55\x04\x03.{1,5}([\x20-\x7e]{3,64})", signature_data
        )
        if cn_match:
        ...
        email_match = re.search(
            rb"[\x20-\x7E]{3,64}@[\x20-\x7E]{3,64}\.[a-zA-Z]{2,10}", signature_data
        )
        if email_match:
    ...
    try:
        ...
        for pattern, label in patterns:
            ...
            for match in matches:
                try:
                    creator_info.append(
                        f"{label}: {match.decode('utf-8', errors='replace')}"
                    )
                except:
                    pass
...
def analyze_timestamps(timestamps):
    ...
    if working_hours_count > len(timestamps) * 0.7:
        analysis += (
            "  Most files were modified during typical working hours (9am-6pm)\n"
        )
    ...
    if time_gaps:
        ...
        if files_before > files_after and files_after / len(timestamps) < 0.3:
            ...
            for t in sorted_times:
                if (
                    t > main_cluster_end
                    and (t - main_cluster_end).total_seconds() > 86400
                ):
...
def find_common_paths(path_parts):
    ...
    recurring = [
        part for part, count in path_parts.items() if count > 1 and len(part) > 2
    ]
