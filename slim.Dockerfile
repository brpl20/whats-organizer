...
RUN apt-get install -y --no-install-recommends ffmpeg wget libreoffice-writer-nogui poppler-utils fonts-noto-color-emoji
...
for FLASK_PORT in $(seq ${FLASK_PORT_START} ${FLASK_PORT_END}); do \
  gunicorn --preload -w 1 -k geventwebsocket.gunicorn.workers.GeventWebSocketWorker -b 0.0.0.0:${FLASK_PORT} app:app --timeout 300 --limit-request-line 0 & \
done && wait"]
